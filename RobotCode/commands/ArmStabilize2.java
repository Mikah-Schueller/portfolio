...
public class ArmStabilize2 extends CommandBase {
   ...
   */
  public ArmStabilize2(WhackAB2 climber) {
  ...
  public void execute() {
   ...
   if (current > start){
    arm.retractClimber(Math.abs(start-current)*0.45);
    arm.display();
   }
   else if (current < start){
    arm.extend(Math.abs(start-current)*0.45);
    arm.display();
   }
   else if (current == start){
    arm.stop();
